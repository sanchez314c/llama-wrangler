...
def main():
    ...
    # FIX: Validate quantization type to prevent shell injection downstream
    if quantization not in VALID_QUANT_TYPES:
        print(f"Error: Invalid quantization type '{quantization}'")
        sys.exit(1)

    # FIX: Validate model_id to safe characters (namespace/repo format)
    import re
    if not re.match(r'^[a-zA-Z0-9._\-/:]+$', model_id) or len(model_id) > 500:
        print("Error: Invalid model ID format")
        sys.exit(1)

    # FIX: Resolve output_dir and ensure it exists under home to prevent path traversal
    output_dir = str(Path(output_dir).resolve())
    home_dir = str(Path.home())
    if not output_dir.startswith(home_dir):
        print("Error: Output directory must be within the home directory")
        sys.exit(1)
