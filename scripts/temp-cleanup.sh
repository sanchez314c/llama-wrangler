...
print_header "🧹 LLAMA WRANGLER TEMP CLEANUP"
...
# Clean Llama Wrangler specific temp files
print_status "🦙 Cleaning Llama Wrangler specific artifacts..."

# Clean any temporary model downloads or conversion artifacts
for temp_pattern in "*.tmp" "*.downloading" "temp_*" ".llamawrangler_temp*"; do
    if ls $temp_pattern 2>/dev/null; then
        print_info "Removing temporary files: $temp_pattern"
        rm -f $temp_pattern 2>/dev/null || true
    fi
done

# Summary and recommendations
print_header "📊 CLEANUP SUMMARY & RECOMMENDATIONS"

print_info "Regular maintenance tasks:"
print_info "  • Run temp cleanup weekly: ./scripts/temp-cleanup.sh"
...
print_info "Llama Wrangler specific:"
print_info "  • Clean model download cache periodically"
print_info "  • Remove failed conversion artifacts"
print_info "  • Monitor ~/.llama-wrangler directory size"
...
print_info "🔍 For detailed bloat analysis, run: ./scripts/bloat-check.sh"